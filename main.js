class Main {
    ...
    update(delta) {
        this.screens[this.screen].update(delta);
    }

    draw() {
        himi_js.clear_color("rgb(0, 6, 71)")
        this.screens[this.screen].draw();
    }
}
...
class TitleScreen{
    ...
    update(delta) {
        if (himi_js.key_down("Enter") || himi_js.mouse_clicked) {
            this.screen.screen = "play"
        }
    }
    draw() {
        himi_js.draw_text("Himi Shooter", himi_js.width / 2, himi_js.height / 2, 130, "rgb(227, 147, 0)");
        himi_js.draw_text("エンターキーか画面タップでスタート", himi_js.width / 2, himi_js.height / 2 + 200, 40, "rgb(227, 147, 0)");
    }
}
...
class Game_Over{
    ...
    update() {
        if (himi_js.key_down("Enter") || himi_js.mouse_clicked) {
            this.screen.screens.play = new PlayScreen;
            this.screen.screen = "play"
        }
    }
...
class Player{
    ...
    take_damage() {
        ...
        this.lives -= 1;
        if (this.lives < 0) {
            this.screen.screen.screen = "gameover"
        }
